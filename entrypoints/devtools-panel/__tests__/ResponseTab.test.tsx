...
describe('ResponseTab', () => {
  ...
  describe('Body section', () => {
    it('renders "Body" heading when response is present', () => {
      render(
        <ResponseTab request={makeRequest({ response: '{"data":{"hero":{"name":"Luke"}}}' })} />
      )
    ...
    it('falls back to <pre> display when response is a JSON array', () => {
      render(<ResponseTab request={makeRequest({ response: '[1,2,3]' })} />)
      expect(screen.queryByTestId('json-view')).not.toBeInTheDocument()
      const pres = document.querySelectorAll('pre')
